...
function orderNowViaWhatsApp(productId, quantity) {
  ...
  const order = {
    ...
    status: 'قيد الانتظار'
     ...
     if (typeof MAHFOOR_FIREBASE !== 'undefined') {
      MAHFOOR_FIREBASE.saveOrder(order);
    } else {
      orders.push(order);
...
function forceClearOrders() {
  localStorage.setItem('mahfourOrders', JSON.stringify([]));
  if (typeof MAHFOOR_FIREBASE !== 'undefined') {
    MAHFOOR_FIREBASE.clearAllOrders();
  }
...
// Initialize
function initialize() {
  // ← Firebase: تهيئة عند تحميل الصفحة
  if (typeof MAHFOOR_FIREBASE !== 'undefined') {
...
// 1. منع كليك يمين
    document.addEventListener('contextmenu', function(e) {
        e.preventDefault();
        // alert('عفواً، تم تعطيل زر الفأرة الأيمن لحماية المحتوى!');
    });

    // 2. منع أزرار واختصارات الـ Inspect
    document.addEventListener('keydown', function(e) {
        // منع زر F12
        if (e.key === 'F12') {
            e.preventDefault();
            // alert('عفواً، أدوات المطورين معطلة!');
            return false;
        }
        ...
        // منع Ctrl + U (عرض السورس كود)
        if (e.ctrlKey && e.key === 'u' || e.ctrlKey && e.key === 'U') {
            e.preventDefault();
            // alert('عفواً، عرض سورس الصفحة معطل!');
            return false;
        }
    });